...
int main() {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        TPoint B1, B2;
        if (vtx.size() > 1) {
            B1 = TPoint(vtx[0].x - (vtx[1].x - vtx[0].x), vtx[0].y - (vtx[1].y - vtx[0].y));
            B2 = TPoint(vtx.back().x - (vtx[0].x - vtx.back().x), vtx.back().y - (vtx[0].y - vtx.back().y));
        } else {
            B1 = TPoint(vtx[0].x - a[i].GetB(), vtx[0].y + a[i].GetA());
            B2 = TPoint(vtx[0].x + a[i].GetB(), vtx[0].y - a[i].GetA());
            if (B1 < B2) swap(B1, B2);
        }
        cerr << endl;
        int B1Id = Point.size();
        ...
        for (int i = 0; i < vtx.size(); ++i) {
            if (i > 0)
                AddEdge(GetVertex(vtx[i]), GetVertex(vtx[i - 1]));else
                AddEdge(GetVertex(vtx[i]), B1Id);

            if (i  + 1 < vtx.size())
                AddEdge(GetVertex(vtx[i]), GetVertex(vtx[i + 1]));else
                AddEdge(GetVertex(vtx[i]), B2Id);
        }
        cerr << endl;
        cerr << endl;
    }
...
//            cerr << Point[vj] << " - ";
            ...
            while (y != i) {
                ...
                int pk = 0;
                for (int k = 0; k < v.size(); ++k) if (v[k].second == x) {
                    pk = k;
                    break;
                }
                if (pk == 0) pk = v.size() - 1;else --pk;
                if (BadPoint[v[pk].second]) {
                    bad = true;
                    break;
                }
                
                cerr << "clock: " << Point[x] << " " << Point[y] << ": ";
                for (int k = 0; k < v.size(); ++k) cerr << v[k].first << " ";
                
                cerr << " -> " << v[pk].first << endl;
