...
#define PUSH(t)   q.push(make_pair(min(min(a[t.first.first][t.first.second], a[t.first.first][t.second.second]), min(a[t.second.first][t.first.second], a[t.second.first][t.second.second])), t))
...
int main() {
    ...
    q.push(make_pair(0, make_pair(make_pair(0, 0), make_pair(n - 1, m - 1))));

    LL ans = 0;
    int it = 0;
    while (!q.empty() && ++it <= 5000000) {
        pair< pair<int, int>, pair<int, int> > t = q.top().second; q.pop();
        int m = min(
